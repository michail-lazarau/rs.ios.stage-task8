...
#import "CanvasDrawingView+Landscape.h"
...
-(void)drawLandscape {
	...
	self.shapeLayers = [[NSArray alloc] initWithObjects:[CAShapeLayer layer], [CAShapeLayer layer], [CAShapeLayer layer], nil];
	for (int i = 0; i < self.shapeLayers.count; i++) {
		UIBezierPath* path = [self landscapeDrawings][i];
		self.shapeLayers[i].path = path.CGPath;
		self.shapeLayers[i].fillColor = [UIColor clearColor].CGColor; // prevent the shape layer from filling
		[self.layer addSublayer:self.shapeLayers[i]];

		if (i + 1 > self.colorsSelected.count) {
			self.shapeLayers[i].strokeColor = [UIColor blackColor].CGColor;
		} else {
			self.shapeLayers[i].strokeColor = self.colorsSelected[i].CGColor; // цвет линии рисования
		}

		self.shapeLayers[i].strokeStart = 0.0; // начало пути относительно координат stroke?
		[self.shapeLayers[i] addAnimation:[[self class] setupDrawsAnimation] forKey:nil];
		path = nil;
	}
}
